...
public class WorkOrderServiceImpl implements WorkOrderService {
    ...
    public synchronized boolean add(Integer batchId) throws BaseException {
        ...
        //地块面积
        Map<String,Integer> map = cropBatchMapper.getCropIdAndCalIdByBatchId(batchId);
        List<Lot> lots = lotMapper.getLotByBatchId(batchId);
        BigDecimal acreageSum = lotMapper.getScaleSum(map.get("cropId"),map.get("calId"),lots);
        for (Farming farming : farmingList){
            ...
            //最少人数
            Short minPerson = acreageSum.divide(personnelUnit,4,BigDecimal.ROUND_UP).divide(new BigDecimal(farming.getFarmingCycle()),0,BigDecimal.ROUND_UP).shortValue();
            //开始时间
            ...
            if (deviceInfoList != null && !deviceInfoList.isEmpty()){
                ...
                for (DeviceInfo deviceInfo : deviceInfoList) {
                    ...
                    //最小人数
                    prepareItem.setMinPerson(minPerson);
                    //数量
                    BigDecimal tempNum = acreageSum.divide(deviceInfo.getUnitMu(),4).setScale(0,BigDecimal.ROUND_UP);
            ...
            if (personnelInfoList != null && !personnelInfoList.isEmpty()){
                for (PersonnelInfo personnelInfo : personnelInfoList) {
                    ...
                    prepareItem.setEndTime(Date.from(endDateTime.toInstant()));
                    prepareItem.setMinPerson(minPerson);
                    BigDecimal tempNum = acreageSum.divide(personnelInfo.getUnitMu(),4).setScale(0,BigDecimal.ROUND_UP);
                    prepareItem.setNum(tempNum.intValue());
    ...
    public boolean addTemporaryOrder(Integer[] batchIds,Integer farmingId) throws BaseException {
        ...
        for (Integer batchId : batchIds) {
            ...
            //批次地块总规模
            List<Lot> lots = lotMapper.getLotByBatchId(batchId);
            BigDecimal acreageSum = lotMapper.getScaleSum(farming.getCropId(),farming.getCalId(),lots);
    ...
    @Override
    public Map<String, Integer> getOrderCount(Integer baseId) {
        return workOrderMapper.getOrderCount(baseId);
    }
