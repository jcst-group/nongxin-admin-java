...
public class EnvMonitorController {
    ...
    public Result<PageInfo> getEnvMonitorList(@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
                                              ...
                                              @RequestParam(defaultValue = "1") Integer factoryId,Integer equipmentId,
                                              Integer baseId, Date startTime, Date endTime){
        Result<PageInfo> result = new Result<>();
        PageHelper.startPage(pageNo,pageSize);
        List<EnvMonitor> list = envMonitorService.getEnvMonitorList(factoryId,equipmentId, baseId, startTime, endTime);
        PageInfo pageInfo0 = new PageInfo(list);
        result.setSuccessResult(pageInfo0);
        return result;
    }
    ...
    public Result<AppEnvVo> getEnvChart(Integer equipmentId,Integer baseId,Date startTime,Date endTime){
        ...
        Integer factoryId = equipmentService.getFactoryId(equipmentId); //取得厂商类型

        List<EnvMonitor> envMonitorList = envMonitorService.getEnvChart(factoryId,equipmentId,baseId,startTime,endTime);
